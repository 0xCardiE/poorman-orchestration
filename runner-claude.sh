...
echo "Anthropic Claude runner starting..."

if ! command -v claude >/dev/null 2>&1; then
  echo "Anthropic Claude CLI (claude) not installed or not in PATH."
  exit 1
fi
...
if [ -z "$(ls -A "$TODO_DIR" 2>/dev/null)" ]; then
  echo "No tasks in tasks/todo/. Asking Claude to generate initial tasks..."
  claude --workspace "$ROOT_DIR" "
...
$(cat "$task_file")"
  ...
  if ! claude --workspace "$ROOT_DIR" "$prompt" | tee "$log_file"; then
    echo "Task failed"
    mv "$task_file" "$FAILED_DIR/"
    git -C "$ROOT_DIR" add -A || true
    git -C "$ROOT_DIR" commit -m "claude failed $task_name" || true
    return 1
  fi
  ...
  git -C "$ROOT_DIR" add -A || true
  git -C "$ROOT_DIR" commit -m "claude completed $task_name" || true
}
...
while true; do
  ...
  if [ ${#TASKS[@]} -eq 0 ]; then
    echo "No tasks left. Asking Claude if more tasks are needed..."
    claude --workspace "$ROOT_DIR" "
